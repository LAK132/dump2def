...
{
	std::cout
	  << "dump2def - create a module definitions file from a dumpbin file\n"
	     "           Written and placed in public domain by Jeffrey Walton\n"
	     "\n"
	     "Usage: \n"
	     "  dump2def <infile>\n"
	     "    - Create a def file from <infile> and write it to a file with\n"
	     "      the same name as <infile> but using the .def extension\n"
	     "  dump2def <infile> <outfile>\n"
	     "    - Create a def file from <infile> and write it to <outfile>\n";
...
int main(int argc, char *argv[])
{
	...
	for (size_t i = 0; i < argc; ++i) opts.push_back(argv[i]);
	...
	if (opts.size() == 2)
	{
		...
		std::string::size_type pos =
		  outfile.length() < 5 ? std::string::npos : outfile.length() - 5;
	...
	// Check or exit
	if (opts.size() != 3) PrintHelpAndExit(1);
	...
	try
	{
		...
		outfile << "DESCRIPTION \"C++ Library\"" << std::endl;
		...
		for (; it != symbols.end(); ++it) outfile << "\t" << *it << std::endl;
	}
	catch (const std::exception &ex)
	{
		std::cerr << "Unexpected exception:" << std::endl;
